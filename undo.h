...
void undo_one_player_odd(cell *list,int start,int cell_number,struct cell_map game_map[][size+2])
{
    FILE *save_f=fopen("D://undo//undo_odd.bin","wb");
...
void undo_one_player_even(cell *list,int start,int cell_number,struct cell_map game_map[][size+2])
{
    FILE *save_f=fopen("D://undo//undo_even.bin","wb");
    fwrite(&start,sizeof(int),1,save_f);
    fwrite(&cell_number, sizeof(int),1,save_f);
    cell *current=list;
    while(current!=NULL)
